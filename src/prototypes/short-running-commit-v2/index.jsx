...
function CommitPanelContent({ context }) {
  ...
  const changesEmpty = files.find(g => g.id === 'changes')?.children.length === 0;

  return (
    <div ref={panelRef} className={`commit-panel-wrapper commit-window-custom${commited ? ' is-commited' : ''}${changesEmpty ? ' changes-empty' : ''}${noFilesChecked ? '' : ' has-checked-files'}`}>
      <CommitWindow
        files={files}
...
export const SCREEN_GROUPS = [
  {
    ...
    screens: [
      { id: 'commit-n0102', label: 'Default' },
      { id: 'src-v2-button-animation', label: 'Commit button animation' },
    ],
  },
];

export default function ShortRunningCommitV2Screen({ screenId }) {
  ...
  if (screenId === 'src-v2-button-animation') {
    return (
